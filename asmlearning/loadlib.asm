...
xor r15, r15                      ;function counter #2
js loadlib
...
;=====================================================
OrdinalLookupSetup:  ;We found our target WinApis
   pop r15         ;getprocaddress
   add rsp, 8
   mov r14, rdi    ;loadlibrary 
   xor rdi, rdi
   js OrdinalLookup
...
; Get function address from AddressOfFunctions
   ...
   add rax, r8                   ; Found the official full WinApi address!!!
...
; Save the handle returned in RAX
mov rdi, rax                     ; Move handle for user32.dll to RDI for later use
add rsp, 0x30
getprocaddress:
...
add rsp, 0x30
mov r15, rax                     ;holds User32.MessageBoxA

messageboxfinally: 
    mov rcx, 0                     ; hWnd = NULL (no owner window)
	mov rax, 0x006D                ;m, 0
	push rax
	mov rax, 0x6574737973743367    ;g3tsyst3
	push rax
